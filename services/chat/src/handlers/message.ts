 ...
 */
export function registerMessageHandlers(socket: ServerSocket, io: ServerInstance): void {
  ...
  socket.on("send-message", async ({ tradeRoomId, content }) => {
    ...
    try {
      ...
      const roomResult = await pool.query<{ seller_id: string; buyer_id: string; status: string }>(
        `SELECT "sellerId" AS seller_id, "buyerId" AS buyer_id, status
         FROM trade_room
         WHERE id = $1
         LIMIT 1`,
        [tradeRoomId]
